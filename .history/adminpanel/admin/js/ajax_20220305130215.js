...
$(document).on("submit","#adminLoginFrm", function(){
   $.post("query/loginExe.php", $(this).serialize(), function(data){
      if(data.res == "invalid")
      {
        Swal.fire(
          'ผิดพลาด',
          'Username หรือ Password ของคุณไม่ถูกต้อง',
          'error'
        )
      }
...
$(document).on("submit","#addCourseFrm" , function(){
  $.post("query/addCourseExe.php", $(this).serialize() , function(data){
  	...
  	else if(data.res == "สำเร็จ")
  	{
  		Swal.fire(
  			'Success',
  			'เพิ่มรายวิชา'+data.course_name.toUpperCase()+'เรียบร้อยแล้ว' ,
  			'success'
  		)
          // $('#course_name').val("");
          refreshDiv();
     ...
     $.ajax({
      ...
      success : function(data){
        if(data.res == "success")
        {
          Swal.fire(
            'เสร็จสิ้น',
            'ลบรายวิชา'+ data.course_name.toUpperCase() +'เรียบร้อยแล้ว',
            'success'
          )
          refreshDiv();
        }
...
$(document).on("click", "#deleteExam", function(e){
    ...
    $(document).on("click", "#deleteExamne", function(e){
      e.preventDefault();
      var id = $(this).data("id");
       $.ajax({
        type : "post",
  ...
  $.post("query/addExamExe.php", $(this).serialize() , function(data){
    ...
    if(data.res == "noDisplayLimit")
   {
    ...
    else if(data.res == "สำเร็จ")
    {
      Swal.fire(
        'Success',
        data.examTitle.toUpperCase() + '<br>เพิ่มข้อสอบเรียบร้อยแล้ว',
        'success'
      )
          $('#addExamFrm')[0].reset();
          $('#course_name').val("");
...
$(document).on("submit","#addQuestionFrm" , function(){
  $.post("query/addQuestionExe.php", $(this).serialize() , function(data){
    ...
    else if(data.res == "success")
    {
      Swal.fire(
        'Success',
         data.msg + ' คำถาม <br>เพิ่มคำถามเรียบร้อยแล้ว',
        'success'
      )
...
$(document).on("submit","#addExamineeFrm" , function(){
  $.post("query/addExamineeExe.php", $(this).serialize() , function(data){
    ...
    else if(data.res == "สำเร็จ")
    {
      Swal.fire(
          'Success',
          data.msg + ' เพิ่มนักเรียนเรียบร้อยแล้ว!',
          'success'
       )
        refreshDiv();
...
$(document).on("submit","#updateExamineeFrm" , function(){
  $.post("query/updateExamineeExe.php", $(this).serialize() , function(data){
     if(data.res == "สำเร็จ")
     {
        Swal.fire(
            'สำเร็จ',
            data.exFullname + ' <br>อัพเดทนักเรียนเรียบร้อยแล้ว!',
            'success'
          )
          refreshDiv();
     }
